...
def load_pretrained_model(model_path, model_base, model_name, model_type, load_8bit=False, load_4bit=False,
    ...
    if model_type not in {'phi-1.5', 'phi-2', 'phi-3', 'stablelm-2', 'qwen1.5-1.8b', 'minicpm', 'llama3.1-8b'}:
    ...
    if 'lora' in model_name.lower() and model_base is not None:
        ...
        if model_type == 'phi-1.5' or model_type == 'phi-2':
            ...
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=True)
            model = CapxLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True,
                                                          config=lora_cfg_pretrained,
                                                          **kwargs)
        ...
        if model_type == 'phi-1.5' or model_type == 'phi-2':
            ...
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=True)
            model = CapxLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained,
                                                          **kwargs)
        ...
        if model_type == 'phi-1.5' or model_type == 'phi-2':
            ...
            tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=True)
            model = CapxLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
